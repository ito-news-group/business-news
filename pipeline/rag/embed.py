...
def get_unembedded_articles(client, only_today: bool = True) -> list:
    ...
    try:
        ...
        if only_today:
            query = query.gte("published_at", f"{today}T00:00:00+00:00")
        rows = []
        offset = 0
        while True:
    ...
    if not rows:
        logger.info("Islenmemis article yok.")
        return []

    rows = [r for r in rows if r.get("full_text") and len(r["full_text"]) >= MIN_TEXT_LEN]

    ids = [r["id"] for r in rows]
    embedded_ids = set()
    id_offset = 0
    while id_offset < len(ids):
        id_chunk = ids[id_offset:id_offset + 100]
        try:
            existing = (
                client.table("article_embeddings")
                .select("article_id")
                .eq("model_version", EMBED_MODEL_VERSION)
                .in_("article_id", id_chunk)
                .execute()
            )
            for row in existing.data:
                embedded_ids.add(row["article_id"])
        except Exception as e:
            logger.warning(f"embed kontrol hatasi: {e}")
        id_offset += 100

    unembedded = [r for r in rows if r["id"] not in embedded_ids]
    logger.info(f"{len(unembedded)} islenmemis article (toplam {len(rows)} kontrol edildi)")
    return unembedded
...
def _upsert_batch(client, rows: list[dict]) -> int:
    ...
    try:
        ...
        logger.error(f"Upsert hatasi, insert fallback deneniyor: {e}")
        try:
