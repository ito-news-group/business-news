...
def fulltext_search(query: str, limit: int, client):
    ...
    if not clean_q:
        return []
    try:
        result = client.rpc("fulltext_search_articles", {
            "query_text": clean_q,
            "match_count": limit,
        }).execute()
        return result.data or []
...
async def rag_metrics():
    ...
    return {
        ...
        "window_300s": {
            "requests": len([m for m in _metrics_window if now - m["ts"] < 300]),
            "errors": len([m for m in _metrics_window if now - m["ts"] < 300 and m["status"] != "ok"]),
        },
